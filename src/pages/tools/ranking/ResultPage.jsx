...
const Container = styled.div`
    padding: 2rem;
    @media (max-width: 560px) { padding: 0.75rem; }
`;
...
const IndexBadgeDesktop = styled.div`
    ...
    opacity: 0.8;
    @media (max-width: 900px) { width: 32px; }
...
const CoverWrap = styled.div`
    ...
    background: rgba(255,255,255,0.06);
    flex: 0 0 auto;
    @media (max-width: 900px) { width: 60px; height: 60px; }
    @media (max-width: 560px) { width: 56px; height: 56px; }
    @media (max-width: 360px) { width: 48px; height: 48px; border-radius: 6px; }
`;
...
const IndexBadgeMobile = styled.div`
    ...
    font-variant-numeric: tabular-nums; font-size: 12px; line-height: 1;
    background: rgba(0,0,0,0.6); color: white;
    display: none;
    @media (max-width: 560px) { display: inline-block; }
    @media (max-width: 360px) { font-size: 11px; padding: 2px 5px; }
`;

const Meta = styled.div` min-width: 0; display: grid; gap: 4px; `;
const Title = styled.div`
    font-weight: 700; font-size: 1rem; line-height: 1.2;
    overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
    @media (max-width: 560px) { font-size: 0.95rem; }
    @media (max-width: 360px) { font-size: 0.9rem; }
`;
const Artists = styled.div`
  font-size: 0.9rem; opacity: 0.8;
  overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
  @media (max-width: 560px) { font-size: 0.85rem; }
  @media (max-width: 360px) { font-size: 0.8rem; }
`;
...
export default function ResultPage({ token, rankedTracks }) {
		...
		const tracks = useMemo(() => {
				...
				try { return JSON.parse(sessionStorage.getItem("si_rankedTracks") || "[]"); }
				catch { return []; }
		}, [rankedTracks]);
		...
		const baseName = useMemo(
				() => (sessionStorage.getItem("si_selectedPlaylistName")?.trim() || "Ranked Playlist"),
				[]
		);
				...
				.then(user => {
						return fetch(`https://api.spotify.com/v1/users/${user.id}/playlists`, {
								...
								body: JSON.stringify({
										...
										description: "Sorted with SoundIndex – Ranking tool",
										public: false
								})
						})
						.then(res => res.json())
		...
		return (
				<Container>
						...
						<ActionBar>
								...
								<PrimaryBtn onClick={createRankedPlaylist}>Create Playlist on Spotify</PrimaryBtn>
