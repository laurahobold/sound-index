...
export default function SortPage ({sortingStack, setSortingStack, setRankedTracks}) {
		...
		function ChoiceCard ({track, label}) {
				...
				return ( <Card bg={track?.cover}>
						<ThumbWrap>
								{track?.cover ? ( <Thumb src={track.cover} alt={track?.name || "cover"} loading="lazy"/> ) : (
						...
						{/* keep audio or other controls in the grid row "audio" */}
				</Card> );
		}
		...
		return ( <Wrap>
				...
				<Wrapper>
						...
						</ChoicesGrid> ) : ( <Merging> Merging… </Merging> )}
						<ProgressWrap>
								<ProgressTrack>
...
const Card = styled.div`
    ...
    &::before {
        ...
        /* slightly scale so the blur doesn't show edges */
    ...
    /* semi-opaque tint layer for contrast + webkit fallback */
    ...
    &::after {
        ...
        pointer-events: none;
        /* optional: give inner content a backdrop-filter for glass effect */
...
/* ThumbWrap and Thumb sit above the blurred layers (z-index: 2) */
const ThumbWrap = styled.div`
    ...
    display: grid;
    place-items: center;
    z-index: 2; /* above the ::before/::after layers */
...
/* bump CardText above visual layers too */
const CardText = styled.div`
    ...
    /* add a subtle text-shadow for extra readability on very noisy covers */
